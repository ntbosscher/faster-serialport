...
function getBinding() {
  ...
  const candidates = [
    // shipped prebuild for this platform/arch, staged by scripts/copy-prebuild.js
    path.join(__dirname, '..', 'prebuilds', process.platform + '-' + process.arch, 'faster-serialport.node'),
    // local build outputs (npm run build during development)
...
class SerialBinding {
  ...
  async open(path, options) {
    ...
    // eventsCallback is a persistent per-port callback (native fires it for the
    // port's lifetime), so it's passed as its own argument rather than through
    // the JSON-encoded options.
    this.fd = await asyncOpen(path, this.openOptions, this.openOptions.eventsCallback)
  }
