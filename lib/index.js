...
class FastSerial {
    ...
    async read(nBytes) {
        // allocUnsafe is safe here: the buffer is fully overwritten by the read
        // and sliced to bytesRead (see bindings.js read), so no uninitialized
        // bytes are ever exposed. Concurrent reads are serialized by Go's read
        // conch.
        return this.binding.read(Buffer.allocUnsafe(nBytes), 0, nBytes);
    }

    bufferedRead(callback, timeout = 1000) {
        return this.binding.bufferedRead(callback, timeout);
    }

    bufferedReadExt(callback, opts = {}) {
        return this.binding.bufferedReadExt(callback, opts);
    }
    ...
    async write(buf, echoMode = false) {
        ...
        // Go serializes writes on the write conch, so a write can proceed
        // concurrently with an in-flight read (full-duplex).
        return this.binding.write(buf, echoMode);
    }
