    ...
    async read(nBytes) {
        return this.queue(async () => {
            ...
            try {
                ...
                // allocUnsafe is safe here: the buffer is fully overwritten by the
                // read and sliced to bytesRead (see bindings.js read), so no
                // uninitialized bytes are ever exposed.
                const i = await this.binding.read(Buffer.allocUnsafe(nBytes), 0, nBytes);
                this.oneAtaTime = false;
    ...
    bufferedReadExt(callback, opts = {}) {
        return this.queue(async () => {
            ...
            try {
                ...
                this.oneAtaTime = true;
                const i = await this.binding.bufferedReadExt(callback, opts);
                this.oneAtaTime = false;
